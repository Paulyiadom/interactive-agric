...
// let totalBaseProduction = [];
let totalProductionPerProduct;
let productsInfo = []; // Array to hold product information
...
async function fetchBaseProduction() {
    try {
        const response = await fetch('/base-production'); // Fetch from the new endpoint
...
// Calculate total farmland size if region exists
const afir =  region.AffordabilityindexbyRegion ;
        ...
        const tooltipContent = `
            <div class="tip-content">
                ...
                <ul>
                    ...
                    <li>
                        <mark>AffordabilityIndex: </mark>
                        <small>${afir.toFixed(3)}</small>
                    </li>
                </ul>
...
function createTopButtons() {
    const majorProduct = ['Palm oil', "maize", "cocoa", "Tilapia", "coffee", "Yam", "beans","coconut", "Avocados", "Mango", "cattle", "sheep", "goat", "Turkey", "Onions"];
...
async function calculateGrowthRate(productName) {
    const totalBaseProduction = await fetchBaseProduction();
    productName = productName.toLowerCase();

    // Find base data
    const baseData = totalBaseProduction.base.find(item => item.Production.toLowerCase() === productName);
    if (!baseData) throw new Error(`Base data for "${productName}" not found.`);
    
    // Find current data
    const currentData = productsInfo.find(p => p.name.toLowerCase() === productName);
    if (!currentData) throw new Error(`Current data for "${productName}" not found.`);

    const totalSum = Object.values(currentData.regions).reduce((sum, value) => sum + value, 0);
    
    const previousValue = baseData.Value * 1000;
    if (previousValue === 0) return null;

    const growthRate = ((totalSum / previousValue) - 1) * 100;
...
function projectToYear(currentValue, growthRate, year) {
    const currentYear = new Date().getFullYear();
    const yearsToProject = year - currentYear;
    return currentValue * Math.pow(1 + (growthRate / 100), yearsToProject) * 1000;
}

// Function to plot the projection chart
// Plot the chart
async function plotProjectionChart(productName) {
    try {
        const totalBaseProduction = await fetchBaseProduction();
        const baseData = totalBaseProduction.base.find(item => item.Production.toLowerCase() === productName.toLowerCase());
        if (!baseData) throw new Error(`Base data for "${productName}" not found.`);

        const currentData = productsInfo.find(p => p.name.toLowerCase() === productName.toLowerCase());
        if (!currentData) throw new Error(`Current data for "${productName}" not found.`);

        // Calculate total current production from regional data
        const totalSum = Object.values(currentData.regions).reduce((sum, value) => sum + value, 0);

        // Calculate growth rate and project future production
        const growthRate = ((totalSum / baseData.Value) - 1) * 100;
        const projectedValue = projectToYear(totalSum, growthRate, 2030);

        // Chart.js setup
        const ctx = document.getElementById('fssrChart');
        if (!ctx) {
        ...
        const labels = ['Base Production (2016)', 'Current Production (2017)', 'Projected Production (2030)'];
        const dataBase = [baseData.Value, null, null];
        const dataCurrent = [null, totalSum, null];
        const dataProjected = [null, null, projectedValue];
        ...
        const projectionChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: `Production Growth for ${productName}`
                    }
                },
                scales: {
                    y: {
...
async function displayAllProducts(regionName, productsInfo) {
    ...
    tableContainer.innerHTML = createHeaderRow();
    ...
    productsInfo.forEach(product => {
        ...
        if (product.regions && typeof product.regions === 'object') {
            ...
            const rowElement = htmlToElement(row);
...
// sum total product 
function sumRegionsByProductName(productName) {
    ...
    const totalSum = Object.values(regions).reduce((sum, value) => sum + value, 0);
    return totalSum;
}
